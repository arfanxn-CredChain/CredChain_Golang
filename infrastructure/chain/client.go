...
func NewClient(p ChainParams) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to bind authority: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create relayer transactor: %v", err)
	}
...
func (c *Client) FetchNonce(ctx context.Context, userWallet common.Address) (*big.Int, error) {
	// Calling the typical userToNonce method generated by abigen
	nonce, err := c.Registry.UserToNonce(&bind.CallOpts{Context: ctx}, userWallet)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch nonce from registry: %v", err)
	}
	return nonce, nil
}
...
func (c *Client) DispatchRelayerTx(ctx context.Context, method func(*bind.TransactOpts) (*types.Transaction, error)) (*types.Transaction, error) {
	// Pre-fetch relayer nonce proactively
	relayerAddr := c.Relayer.From
	nonce, err := c.EthClient.PendingNonceAt(ctx, relayerAddr)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch relayer nonce: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("relayer tx failed: %v", err)
	}
